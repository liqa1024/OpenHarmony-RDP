...
- RDP 引擎：[FreeRDP](https://github.com/FreeRDP/FreeRDP)（源码交叉编译到 `aarch64-linux-ohos` /
  ...
  **live 画面由 FreeRDP gdi 出、经 Vulkan 呈现器上屏**；GPU 引擎（Vulkan）目前只跑 dev 回放/对比，
  「硬件解码」设置项保留但尚未接入 live（见 [`doc_agent/gfx-engine.md`](doc_agent/gfx-engine.md) §5/§7）。
...
- **文档维护**：不要把排查过程、日期、机器/设备型号写进仓库文档。新增结论请写进
  [`doc_agent/`](doc_agent/README.md) 对应的主题文件（写"结论 + 依据 + 约束"，不写尝试过程）。
...
| Progressive kernel 优化（未完成）与新录像正确性残留（**后续工作清单**） | [`gfx-progressive-kernel.md`](doc_agent/gfx-progressive-kernel.md) |
...
| RLGR/解码 kernel 的并行化改造、查新录像的逐像素分叉 | [`gfx-progressive-kernel.md`](doc_agent/gfx-progressive-kernel.md) |
