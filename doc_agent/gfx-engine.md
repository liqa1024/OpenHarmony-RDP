...
6. 服务端等**帧回执**推进下一帧。回执在**解码之后、`EndFrame` 回调之前**就写出（补丁把上游"回调返回后才回执"
   ...
   回执节奏成为可用的流量控制点：**帧率上限**就在 START_FRAME 处闸住下一帧（见 §8.1 与
   [`settings-and-storage.md`](settings-and-storage.md) §2）。回执还带 `queueDepth` = 本机帧流水线
   offload 队列的积压字节数（补丁 34），服务端据此限帧，队列与延迟才有界。
...
- 帧流水线（解压/解析/解码/合成/上屏/回执，以及**限帧的等待**）跑在**会话自己的工作线程**上，不是
...
- 两者都是**阻塞**不是处理 ⇒ 单列、都不进 `本机`；**帧的整段墙钟 = `本机` + `sync` + `presentWait`**
  ...
  - **帧率上限的睡眠同属"阻塞"**：它发生在 START_FRAME、落在同一个 `zgx+parse` 窗口里，同样经
    `OnBlockedBeforeFrameWork()` 摘出 ⇒ `本机` 不含它。所以开着上限时上面那条自检会**左边更小**
    （差额 = 阈值睡眠），这是预期，不是算漏；上限值本身看设置，实际出帧率看 `fps=`。
