...
- **量测纪律（否则数字不可比）**：
  - **两种回放节拍**（`mode=` 字段，dev 页「节拍」按钮切换）：
    - **`mode=fast`（默认，dev 页显示「跑满」）：完全不打节拍**——上一帧做完就喂下一条，`paced=0`。
      **`fps` 就是吞吐上限**（"这些字节喂进来，客户端吃不吃得下"），不是 live 的出帧率。
      ⚠ 这里**曾经**给每帧 16ms 预算（`kFrameMs`、提前做完就补睡余量）：那个 sleep 会把整个 SoC 压到最低
      频率档，**同一份代码的每个 CPU 侧数字都虚高 ~3x**（碎片样本整轮 `cpuKHz=558000-650000` vs 视频样本
      840000-1995000，见 [`cpu-path.md`](cpu-path.md) §8）⇒ 跨样本/跨轮次就没法比了，所以撤掉。
      代价：轻样本会跑到显示/GPU 的上限（碎片样本 ~72fps），那时**帧时间不再只由 CPU 决定**
      （`BeginDesktopBufferWrite` 在那里要等 1.2–3.8ms，视频样本只要 10µs）——A/B 纯 CPU 侧的改动
      请用 CPU 受限的样本（视频），或同轮比较。
      **跑满的轮次之间比较前先看 `run … cpuKHz=`**：两次频率不同就不可比。
    ...
    - **realtime 仍不是闭环**：live 的节奏由服务端与**每帧 `RDPGFX_FRAME_ACKNOWLEDGE`** 共同决定（ack 在
      ...
      版本 0（无时间戳）的旧录像没有到达时刻，`realtime` 会自动回落 `mode=fast`（即跑满）并在日志里说明。
