...
已用此模式的：音频重定向、硬件解码（RFX）。原生只回**稳定原因码**
（`no-vulkan`/`no-instance`/`no-device`/`no-compute`/`no-host-memory`/`no-surface`/`emulator`），
中文文案由 `DeviceCapabilities.hardwareDecode()` 负责；不支持时设置页置灰开关并显示原因，
并把已存的值纠正为关。
...
| 判定 | 用途 | 条件 |
|---|---|---|
| **引擎判定**（`engineSupported`） | 硬件解码 / GPU 回放，最严 | device + **graphics+compute 队列** + host-visible 内存 + `VK_OHOS_surface`/`VK_KHR_swapchain`（Progressive 解码是 compute dispatch） |
| **呈现判定**（`presenterSupported`） | 上屏，较宽松 | device + host-visible 内存 + `VK_OHOS_surface`/`VK_KHR_swapchain`，**不需要 compute** |

- 呈现判定决定用 Vulkan 呈现器还是 GLES 兜底呈现器（`CreateFramePresenter()`）。
