...
- **显示（分辨率/缩放）**：全局可自动（分辨率取当前显示器，缩放按远端分辨率换算）或手动；
  单连接可关掉「使用全局显示设置」用自己保存的 `width/height/scalePercent`。
  连接前用 `SettingsStore.resolveDisplay(conn)` 解析。
  - `分辨率` 是**输出分辨率**（超分辨率后的目标）；超分辨率关闭时它同时就是会话分辨率。
  - `缩放` 是**会话（超分辨率前）桌面**的 DPI 系数。自动档**只由请求的（输出）分辨率确定**
    （`SettingsStore.scalePresetForResolution`）：按**长边**线性换算 `长边 ÷ 1600 × 100`，
    再**吸附**到档位 `100/125/150/175/200/225`（长边 < 1600 即落到 100）。
    例：`1600 → 100`、`1920 → 125`、`2560 → 150`、`2880 → 175`、`3200 → 200`、`3840 → 225`。
...
- **帧率上限**（`maxFps`，每连接，默认跟随全局「连接特性」；档位 0/15/20/30/45/60 fps，0 = 不限制）：
  ...
  上限只降不升：内容本身到不了上限时它无影响。闸门在 **GFX 工作线程**上等待（帧流水线已从 drdynvc
  分发线程搬走，见 [`architecture.md`](architecture.md) §4），所以限帧**不再牵连**音频/输入等其它动态
  通道。同时回执**如实上报本机积压**（`queueDepth`），服务端据此真正降帧——否则队列会无限增长、
  延迟越拖越大。它**只作用于 live 会话**，dev 回放有自己的节拍（`fast`/`realtime`），不叠加。
