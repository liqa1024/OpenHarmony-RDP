...
- **解码并行宽度**（`decodeThreads`，进程级）：Progressive 解码在平台任务队列（ffrt）上的
  `max_concurrency`，0 = 自动（全部在线核，上限 16）、1 = 完全串行（接收线程直接解码，不提交/不唤醒/
  不等待）；2..8 = 手动。设置页有滑条，dev 回放页有「线程」直选行。执行器、宽度语义与判据见
  [`cpu-accel-plan.md`](cpu-accel-plan.md) §0/§1。
- **开发者选项**（`developerMode`，进程级，默认关）：dev/实验功能的**总开关**。关着时以下入口都不存在：
  首页的「回放测试」；设置页的「抓取 RFX 码流（测试）」「解码并行宽度」「触屏高刷新率（实验）」
...
- **解码并行宽度**同属这一类：它是进程级值，解码器在**每条 region 边界**按需读取（不撕掉在飞的
  任务），所以真正生效的时间点是"下一次开始解码"（新会话 / 新回放），切换时不需要重建执行器。
- 窗口尺寸类设置只在**窗口创建时**生效。
