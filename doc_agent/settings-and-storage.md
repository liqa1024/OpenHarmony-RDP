...
- **解码线程数**（`decodeThreads`，进程级）：Progressive 解码的**并行宽度**，0 = 自动（全部在线核，上限 16）、
  1 = 完全串行（接收线程直接解码，不提交/不唤醒/不等待）；2..8 = 手动。设置页有滑条，dev 回放页有
  「线程」直选行。执行器、宽度语义与判据见 [`cpu-accel-plan.md`](cpu-accel-plan.md) §0/§1。
- **开发者选项**（`developerMode`，进程级，默认关）：dev 功能的**总开关**。关着时以下入口都不存在：
  首页的「回放测试」、设置页的「抓取 RFX 码流（测试）」与「解码线程数」、以及会话工具栏的性能遥测
  （网络 / 本机 / FPS / 带宽 / 音频）。关掉它还**一并关闭**已开着的抓取，避免"入口被隐藏、后台还在跑"。
  导入配置**不读取**这个值（回到关），所以导入他人的配置不会意外打开 dev 工具。
...
- **解码线程数**同属这一类：它是进程级值，解码器在**每条 region 边界**应用新值（不撕掉在飞的任务），
