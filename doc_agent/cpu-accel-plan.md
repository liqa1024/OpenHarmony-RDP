...
| # | 项 | 占比 | 做法要点 | 正确性门禁 | 出口 |
|---|---|---|---|---|---|
| **C1** | 逆 DWT 改写（**做完了**） | 4 成半 | 不动算术、只改组织：① `X2` 序列（`L - (H0+H1)/2`）**先整段算出来**，输出对再由一层无依赖的循环写；② `idwt_y` 改成**行主序**（原来按列走）；③ scratch 只有一条行级缓冲。另在 `codec/rfx_dwt.c` 的通用实现上做了等价的 NEON 改写（`VRHADD` = `(a+b+1)>>1`、`VHADD` = `(a+b)>>1` 都是全精度和，与 C 的 `int` 算术逐位相同） | **① 标量 vs 新实现采样对拍**（`mismatch=0`）+ **② `参考:对比` `bad=0`**（见 §7） | `idwt` 绝对量 **−2 成**、`decode`/`本机` 各 **−5%**、整轮 `cpu=` **−4%**（两份录像都过门禁） |
...
- **正确性门禁 = 参考画面（golden reference）**，两份基线录像各一套：先 `参考:导出` 记下
  ...
  - ⚠ **它证明不了"解码改写是逐位等价"**：参考就是被改的那个解码器录的，两边一起变。解码侧（DWT、
    RLGR、去量化）的改写必须自带**标量 vs 新实现的对拍**：patch 里 `HmrdpDwtCheckStat[2]`
    = `{对拍过的 tile 数, 逐元素不同的个数}`，`参考:对比` 那一轮开着（1/16 采样），
    stats 行报 `dwt check: tiles=… mismatch=…`，**`mismatch=0` 是入口**。
    - 对拍必须**先取输入副本再跑新实现**：逆 DWT 的输出就写在系数缓冲上（整个 4096 系数被覆盖），
      在解码之后复制 `buffer` 拿到的是输出，不是输入——那样两边算的不是同一件事。
    - 非"抽取"那一支（`codec/rfx_dwt.c` 的通用实现）在同一对拍里计数；它在全屏 Progressive 码流上
      `tiles=0`，所以对拍要挂在**抽取支的入口**（`rfx_dwt_2d_extrapolate_decode`）才有意义。
...
- **dev 页驱动：别用盲点坐标**。先 `dumpLayout` 取控件 `bounds` 再点中心
  ...
  **一轮没跑完时点击 = 掐断**。
  - ⚠ **「参考」是按 关→导出→对比 循环的，而"停在导出"的那一轮在停止时仍会写盘**：
    要进到「对比」就得路过「导出」，第二次点击的 `stop` 会让那半轮导出把参考覆盖成"跑了十几帧的
    那一次"。所以进「对比」之后**要重新把参考文件推回设备**（本地 `.cache/hmrdp_ref_<tag>.*`），
    再点「重新回放」——`Start` 只在启动时读参考，此时读到的才是好的一份。`refFrames=` 是自检
    （对不上就是被覆盖过）。
