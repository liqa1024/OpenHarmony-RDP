...
| **C2** | `state` + `color` 的逐 tile 缓冲流量（**做完了**） | 共 3 成半 | ② 由 C1b 完成（`yCbCrToRGB` 换 NEON，−57%）；① RLGR **直写 `sign`**、去量化改成 **`sign → buffer`**：那趟 8KB 的 `buffer → sign` 搬运整趟消失（同一批字节改从另一个缓冲读），LL3 因为差分解码要"移位前"的值，仍先拷那 64/81 个样本再就地移位 | ① **逐位等价** ⇒ 对拍与参考都不变（`bad=0` 直接复用，不必重录）；② 属于 C1b 的舍入级口径 | `state` 绝对量 **−5 成**、`decode` **−5%**、`本机` **−5%**、`cpu=` **−4%**、`fps` **+5%** |
| **C3** | `update` 的 tile→surface 拷贝（**试过、被否证**） | 1 成半（C2 之后是 `dec` 里最大的一块） | 计划里的"32 位掩码字 → 64 位掩码字 + 自动向量化"**实测中性**（`update` 8.05 → 8.12ms）⇒ 这个循环的账在内存侧：每 tile 固定 16KB 读源 + 16KB 读目的（保留 alpha）+ 16KB 写目的，且目的按 12.5KB 行距跨行落。细节与那次的判据缺口见 §8.4 | — | 无（改动已撤） |
| — | `upgrade` / `dequant` | 半成 / 百分之几 | 占比小，C1–C3 之后再评估 | — | — |
| — | RLGR | 1 成 | 最小项，最后再看 | — | — |

- **C2 之后 `dec` 的构成**（整屏样本，`prog2` 量级，ms/帧）：`update` 8 ≈ `idwt` 7.3 ≈ `rlgr` 7.3 > `state` 4.9 >
  `color` 4.4 > `upgrade` 2 > `dequant` 1.4。⇒ 剩下的三块**都已经是"字节/访存"的账**：`update` 如上，
  `idwt`/`rlgr` 是每个 tile 固定要过一遍的 4KB 系数。**再要动帧率就该动"每帧过多少字节"**，
  而不是把循环写得更窄（这一轮里两次"把循环写窄"的尝试都在噪声里，见 §8.3/§8.4）。
  ⚠ `rlgr` 在 C2① 之后从 5.7 涨到 7.3ms（同一次重排让它写的是另一个缓冲），净账仍是 −3ms；不要把它读成
  "RLGR 变重了"，它只是相位桶之间的搬运。
- **C2 的两半性质不同**：②（`color`）是**允许舍入**的一类，靠 C1b 顺带拿到；①（`state`）是**逐位等价**
  的一类，`bad=0` 与对拍都直接复用——所以同一项里也能混两种口径，按改动本身判。
...
3. **"`state` 的饱和加是算力瓶颈"** —— 那个循环按元素做两次分支钳位，看着像算力账；换成 `VQADD`
   （8 路、逐位等价的饱和加）后两份样本上 `state` 都在噪声内（±0.2ms）⇒ 它**受搬的字节数限制**
   （工作缓冲与持久 `current` **两个**缓冲都要写），不是受钳位限制。要降它得**少写一趟**
   （C2① 就是把其中一趟整趟删掉），换 SIMD 不解决问题；该 SIMD 改动已撤掉。
   ⇒ 教训：**"看着像算力"的循环，先用一个逐位等价的 SIMD 版本试一次**——它是中性的就说明账在内存侧，
   这时候该动的是数据流而不是指令。
4. **"`update` 的 keep-alpha 拷贝是算力瓶颈"** —— 同样换成 4 像素/指令的字节掩码选择（`VBSL`，
   逐位等价），`update` 8.05 → 8.12ms、`本机` 不变 ⇒ 这个循环的账也在内存侧：**每 tile 固定
   16KB 读源 + 16KB 读目的（保留 alpha 才需要读）+ 16KB 写目的**，且目的按桌面行距跨行落，用满的
   字节只有每行 256B。改动已撤。
   ⇒ 要动它就得动**字节数**（例如"目的 alpha 恒为 0xFF 时省掉那次读"，但那要求先证明该表面从不出现
   非 0xFF 的 alpha——`KEEP_DST_ALPHA` 是上游语义，且 alpha 也在参考哈希里，属**改语义**，不在"舍入"
   的许可范围内，所以先不做）。⚠ 那次实验的**判据没当场证实**：`NEON_INTRINSICS_ENABLED` 依赖 CMake
   生成的 `config.h`（`WITH_SIMD` 在生成头里而不是编译命令行上，见 `prim_copy.c.o` 的 `ninja -t commands`
   输出），所以"NEON 版真的编进去了"这一点没有被独立验证——**下次做这类实验时要先证明"改动确实生效"**
   （把一个可观测的返回值/计数打出来），否则"中性"可能只是"没跑"。
