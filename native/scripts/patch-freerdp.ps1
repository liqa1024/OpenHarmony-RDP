...
# 9) Dev-only state accessor for the offline GFX harness: the per-(tile,component)
#    predictor state (`current`/`sign`/bit positions) lives behind opaque types, so
#    the harness cannot compare the reference decoder's state with another
#    implementation's (see doc_agent/gfx-progressive-kernel.md §3). `bitPos` is
#    written in *band* order (HL1 LH1 HH1 HL2 LH2 HH2 HL3 LH3 HH3 LL3), i.e. the
#    order the bit positions are consumed in, not the struct's declaration order.
$hmrdpTileState = @'
/* HmRdp: dev-only accessor for the per-(tile,component) predictor state. */
FREERDP_API BOOL HmrdpProgressiveTileState(void* ctx, UINT16 surfaceId, UINT16 xIdx, UINT16 yIdx,
                                           const INT16** current, const INT16** sign,
                                           BYTE bitPos[30])
{
	PROGRESSIVE_CONTEXT* progressive = (PROGRESSIVE_CONTEXT*)ctx;
	PROGRESSIVE_SURFACE_CONTEXT* surface = NULL;
	RFX_PROGRESSIVE_TILE* tile = NULL;
	UINT32 zIdx = 0;

	if (!progressive || !current || !sign || !bitPos)
		return FALSE;
	...
	zIdx = ((UINT32)yIdx * surface->gridWidth) + xIdx;
	...
	{
		current[c] = (const INT16*)(&tile->current[((8192 + 32) * c) + 16]);
		sign[c] = (const INT16*)(&tile->sign[((8192 + 32) * c) + 16]);
	}

	{
		const RFX_COMPONENT_CODEC_QUANT* pos[3] = { &tile->yBitPos, &tile->cbBitPos,
			                                        &tile->crBitPos };
		for (size_t c = 0; c < 3; c++)
		{
			bitPos[c * 10 + 0] = pos[c]->HL1;
			bitPos[c * 10 + 1] = pos[c]->LH1;
			bitPos[c * 10 + 2] = pos[c]->HH1;
			bitPos[c * 10 + 3] = pos[c]->HL2;
			bitPos[c * 10 + 4] = pos[c]->LH2;
			bitPos[c * 10 + 5] = pos[c]->HH2;
			bitPos[c * 10 + 6] = pos[c]->HL3;
			bitPos[c * 10 + 7] = pos[c]->LH3;
			bitPos[c * 10 + 8] = pos[c]->HH3;
			bitPos[c * 10 + 9] = pos[c]->LL3;
		}
	}
...
'@
Patch-File "$Source\libfreerdp\codec\progressive.c" @{
  'INT32 progressive_decompress(PROGRESSIVE_CONTEXT* WINPR_RESTRICT progressive,' = $hmrdpTileState + 'INT32 progressive_decompress(PROGRESSIVE_CONTEXT* WINPR_RESTRICT progressive,'
}
