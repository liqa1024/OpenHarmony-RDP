...
//
// Deliberately minimal: the sink is a plain bounded FIFO. It buffers nothing
// beyond the device's own pull cadence, applies no prebuffer/re-prime policy and
// no high-water drop of its own - the only drop point is the ring overflowing
// because playback fell behind, and FreeRDP's own rdpsnd handling is left stock.
// Keeping one layer here is what makes the audio path observable when testing
// whether it is decoupled from the picture pipeline.
//
